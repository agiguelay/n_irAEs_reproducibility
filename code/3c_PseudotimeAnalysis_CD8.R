...
##########################################################
#                                                        #
#     STEP3c: Pseudotime analyis (Slingshot) on CD8      #
#                                                        #
#    Integrates CD8 cells together and run Seurat        #
...
col.cd8 <- setNames(col.palette[12:15], c("CD8 Naive", "CD8 Proliferating", "CD8 TCM", "CD8 TEM"))
...
seurat.all <- subset(seurat.all, subset = Pre_Post == "Post-ICI" & FinalAnnotation %in% c("CD8 Naive","CD8 TCM", "CD8 TEM")) # select the good cells, no proliferating cells because the clycling genes dominate too much
meta.good.cells <- seurat.all@meta.data

# Integration of CD8 T cells ---------------------------------------------------
...
saveRDS(seurat.integrated, paste0(dir.output, "Objects/SeuratIntegrated_CD8.rds"))
...
pdf(paste0(dir.output, "Plots/Pseudotime/UMAP_Seurat_Annotation_CD8.pdf"), width = 12, height = 8)
print(DimPlot(seurat.integrated, reduction = "umap", pt.size = 0.2, shuffle = T, group.by = "FinalAnnotation", label = TRUE,
  ...
  scale_color_manual(values = col.cd8) +
...
pdf(paste0(dir.output, "Plots/Pseudotime/UMAP_Seurat_Clusters_CD8.pdf"), width = 12, height = 8)
...
pdf(paste0(dir.output, "Plots/Pseudotime/UMAP_Seurat_Samples_CD8.pdf"), width = 12, height = 8)
...
sce <- slingshot(sce, clusterLabels = "ClusterInt", reducedDim = reducedDim(sce, 'PCA')[,1:30],
                 allow.breaks = TRUE, start.clus = "2") # Starts in the naive cluster

pdf(paste0(dir.output, "Plots/Pseudotime/Lineage_PCA_CD8.pdf"), width = 12, height = 8)
...
pdf(paste0(dir.output, "Plots/Pseudotime/PT_UMAP_CD8.pdf"), width = 12, height = 8)
plot(reducedDims(sce)$UMAP, col = plotcol, pch=16, asp = 1)
dev.off()

pdf(paste0(dir.output, "Plots/Pseudotime/PT_Linege_UMAP_CD8.pdf"), width = 12, height = 8)
plot(reducedDims(sce)$UMAP, col = plotcol, pch=16, asp = 1)
...
write.table(ATres, paste0(dir.output, "Tables/DynamicGenes_CD8.txt", sep = "\t"))
...
pdf(paste0(dir.output, "Plots/Pseudotime/HeatmapPT_CD8.pdf"), width = 12, height = 15)
Heatmap(mat,
        show_column_names = F,
...
pdf(paste0(dir.output, "Plots/Pseudotime/DistributionPT1_CD8.pdf"), width = 6, height = 4)
...
pdf(paste0(dir.output, "Plots/Pseudotime/DistributionPT1_CD8_CellType.pdf"), width = 6, height = 4)
print(ggplot(df.plot, aes(x = Pseudotime, fill = CellType, color = CellType)) +
        geom_density(alpha= 0.5, aes(fill = CellType)) +
        theme_classic()+
        scale_color_manual(values = col.cd8) +
        scale_fill_manual(values = col.cd8) +
        theme(text = element_text(colour = "black", size = 15),
              aspect.ratio = 1/1,
              axis.line = element_line(color = "black", linewidth = 0.8),
              axis.text = element_text(colour = "black", size = 15),
...
pdf(paste0(dir.output, "Plots/Pseudotime/DistributionPT1_CD8_patient.pdf"), width = 6, height = 4)
...
df <- data.frame(Pseudotime = meta[size.win:(nrow(meta)-size.win),]$Pseudotime,
...
pdf(paste0(dir.output, "Plots/Pseudotime/PropiRAPTRd_CD8.pdf"), width = 15, height = 5)
...
write.table(prop.perm, paste0(dir.output, "Tables/Permutation_PT_CD8.txt"))
...
pdf("Plots/Pseudotime/Prop_irAE_PT_CD8_Permutations_5.pdf", width = 15, height = 5)
print(ggplot(df, aes(x = Pseudotime, y = Prop_irAE)) +
...
df2 <- data.frame(Pseudotime = meta[size.win:(nrow(meta)-size.win),]$Pseudotime,
                 NbTEM = sapply(size.win:(nrow(meta)-size.win), function(x) sum(meta[(x - size.win):(x+size.win),]$FinalAnnotation == "CD8 TEM")),
                 NbTCM = sapply(size.win:(nrow(meta)-size.win), function(x) sum(meta[(x - size.win):(x+size.win),]$FinalAnnotation == "CD8 TCM")),
                 NbNaive = sapply(size.win:(nrow(meta)-size.win), function(x) sum(meta[(x - size.win):(x+size.win),]$FinalAnnotation == "CD8 Naive")))

df2$`CD8 TEM` <- (df2$NbTEM/(2*size.win+1))*100
df2$`CD8 TCM` <- (df2$NbTCM/(2*size.win+1))*100
df2$`CD8 Naive` <- (df2$NbNaive/(2*size.win+1))*100

df3 <- stack(df2[, 5:7])
colnames(df3) <- c("Proportion", "CellType")
df3$Pseudotime <- rep(df2$Pseudotime, 3)

pdf(paste0(dir.output, "Plots/Pseudotime/Prop_Celltype_PT_CD8.pdf"), width = 15, height = 5)
